...
If in-place means that memory consumption does not scale with input size n, then StrategySort satisfies this definition. This is because it uses a fixed [512]int buffer for the quicksort stack, which corresponds to either 2048 B (32-bit) or 4096 B (64-bit), depending on the system architecture. The Mergesort implementation uses a fixed [64]task stack buffer, where each task consists of 3 ints, resulting in 768 B or 1536 B, depending on the architecture. In addition, it uses a [320]T buffer to speed up merging. The size of this generic buffer depends on T and ranges from 320 B (for int8, uint8) to 5120 B (for strings).

One thing worth noting is that the [64]task buffer represents, as is, the limitation of the algorithm in terms of the maximum slice size that can be sorted. With this buffer size, the limit is approximately 5.9 Zettabytes of (u)int8s - a bummer, I know - but this limit can easily be extended by increasing the task buffer to say 128. This should have no real performance impact, as this would only increase the buffer size to 1536 B or 3072 B, respectively.
