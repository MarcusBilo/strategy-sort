...
func StrategySort[T cmp.Ordered](arr []T) {
	...
	for i := 6; i < n; i += 2 {
		if arr[i] < arr[i-2] && arr[i] < arr[i-4] && arr[i] < arr[i-6] {
			posDisorder++
		}
		if arr[i] > arr[i-2] && arr[i] > arr[i-4] && arr[i] > arr[i-6] {
			negDisorder++
...
func mergeIterativeFallback[T cmp.Ordered](a []T, from, pivot, to int, buf *[320]T) {
	...
	for sp > 0 {
		...
		for len1 > 0 && a[from+len1-1] <= a[pivot] {
			len1--
		}
		if len1 == 0 {
			continue
